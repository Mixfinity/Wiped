<?php
	class MProjects extends CI_Model  {
		...
		public function getWorkedMinutesByDate($f_date, $f_user){
			$result = $this->db->query("SELECT pl.name as line_name, p.name as project_name, c.name as contact_name, minutes FROM working_minutes wm
			INNER JOIN project_lines pl ON wm.project_line_id = pl.id
			INNER JOIN projects p ON pl.project_id = p.id
			INNER JOIN contacts c ON p.contact_id = c.id
			WHERE DATE_FORMAT(created_at, '%d-%m-%Y') = ".$this->db->escape($f_date). " AND wm.user_id = " . $this->db->escape($f_user));
			return $result->result();
		}
		...
		public function getSpecificationInformationFromFullProject($f_id = 0){
			$query = $this->db->query("
				SELECT p.name as project_name, c.name as contact_name FROM sub_projects sp
				INNER JOIN projects p ON
				p.id = sp.project_id
				INNER JOIN contacts c ON
				p.contact_id = c.id
				WHERE p.id = " . $this->db->escape($f_id));
		...
		public function getProjectSpecificationsFromFullProjectAdv($f_id = 0, $from = null, $till = null){
			$result = $this->db->query("SELECT 
				DATE_FORMAT(wm.created_at,'%d-%m-%Y') as mutation_date,
				wm.created_at,
				sum(minutes) as minutes,
				...
				INNER JOIN divisions d ON
				d.id = sp.division_id
				WHERE p.id = ".$this->db->escape($f_id)." AND (wm.created_at >= " . $this->db->escape($from) . " AND wm.created_at <= " . $this->db->escape($till) . ")
				GROUP BY  wm.created_at, pl.id, wm.description
				ORDER BY sub_project_name,sub_project_id, division_id, wm.created_at ASC, description");
		...
		public function getProjectSpecificationsFromFullProject($f_id = 0){
			$result = $this->db->query("SELECT 
				DATE_FORMAT(wm.created_at,'%d-%m-%Y') as mutation_date,
				wm.created_at,
				sum(minutes) as minutes,
		...
		public function getWorkReport($date = null, $user = null){
			if($user != null){
				...
				$result = $this->db->query(
					"
						...
						INNER JOIN projects p ON
						p.id = pl.project_id 
						WHERE DATE_FORMAT(wm.created_at, '%d-%m-%Y') =  " . $this->db->escape($date) . "
						AND wm.user_id = " . $this->db->escape($user)
				)->result();
				return $result;
