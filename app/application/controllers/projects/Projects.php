...
class Projects extends CI_Controller {
	public function __construct()
   {
	...
	public function pdf($f_id = 0, $f_from = NULL, $f_till = NULL)
	{
	    ...
	    $data = pdf_create($html, '', false, false);
	...
	public function pdfadv($f_id = 0, $f_from = NULL, $f_till = NULL)
	{
		...
		$till = "";
			...
			if(!empty($f_from) && !empty($f_till)){
				...
				$specifications = $this->project_model->getProjectSpecificationsFromFullProjectAdv($f_id, $from, $till);
			...
			$specifications = $this->project_model->getProjectSpecificationsFromFullProject($f_id);
		}



		$extrainfo = $this->project_model->getSpecificationInformationFromFullProject($f_id);
	...
	public function specifications($f_id = 0, $main = false){
		$this->load->database();
		$this->load->view('common/header');
		$this->load->model("ModelMenu", "menu");
		$temp_menu = $this->menu->getMenu();
		...
		$data = array(
			"specifications" => $specifications,
			"project" => isset($extrainfo->project_name) ? $extrainfo->project_name : false,
			"contact" => isset($extrainfo->contact_name) ? $extrainfo->contact_name : "",
			"from" => $from,
			"till" => $till,
			"id" => $f_id,
			"main" => $main
	...
	function createBaseProject(){
		if($this->input->post('project_name') && $this->input->post('contact_name')){
			...
			if(!empty($this->input->post('need_trello'))) {
				$init = 0;
			}
